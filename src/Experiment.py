...
class Experiment:
    ...
    def read_labels(self, name):
        ...
        for i in range(1, label_file.ID.max() + 1):
            file_exists = os.path.exists(
                self.data_dir.format(name, name.lower()) + str(i) + ".png"
            )
            if not file_exists:
                no_exist.append(i)
        ...
        if self.DEBUG:
            for i in range(1, 28):
                label_file.iloc[:, i] = np.random.choice(
                    [0, 1], size=len(label_file), p=[0.5, 0.5]
                )
    ...
    def run(self, num_labels):
        ...
        for model_name in tqdm(self.model_names):  # run models
            ...
            otb_model, device, input_size = self.initialize_model(
            ...
            for label_num in range(10, 15):
                for name in dataset_names:  # initialize dataloaders
                    ...
                    dataloaders_dict[name] = DataLoader(
                        rd, batch_size=self.batch_size, shuffle=False, sampler=sampler
                    )

                self.dataloaders_dict = dataloaders_dict  # TODO: Clean up
                print(
                    f"Building models for {condition_name}, condition {label_num}/{num_labels}"
                )
                ...
                for learning_rate in self.learning_rates:
                    ...
                    rm.train_model(
                        otb_model, dataloaders_dict, criterion, optimizer, device
                    )
